...
# Use the generativeai package that Instructor officially supports
...
PROMPTS = {
    ...
    "capacity_gaps": "Identify specific operational gaps where the company has scale or complexity that its current tools cannot serve. FOCUS STRICTLY on gaps related to the PRODUCT PROFILES provided below. Look for bottlenecks, manual workflows, or inefficiencies that these specific products solve. If a gap is unrelated to the products, ignore it.",
    "pain_points": "Extract documented pain points causing cost, delay, or compliance risk. FOCUS STRICTLY on pain points related to the PRODUCT PROFILES provided below. Look for issues that align with the ideal customer profile of these products. Distinguish public evidence from inferred signals.",
...
# --- FIX: Configure Gemini and initialize the GenerativeModel ---
genai.configure(api_key=os.getenv("GEMINI_API_KEY"))

model_name = os.getenv("EXTRACTION_MODEL", "gemini-2.5-flash")
raw_client = genai.GenerativeModel(model_name)

# Instructor requires the GenerativeModel instance for Gemini
...
def extract_cluster(
    ...
    try:
        # FIX: Removed 'model' and 'max_tokens'. The model is already bound to the 
        # client in from_gemini, and Gemini handles token limits automatically.
